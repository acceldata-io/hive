...
public class DatabaseProduct implements Configurable {
  ...
  private static final EnumMap<DatabaseProduct.DbType, String> DB_SEED_FN =
      new EnumMap<DatabaseProduct.DbType, String>(DatabaseProduct.DbType.class) {{
        ...
        // ORDER has to be restated: this rewrites the whole identity clause and Oracle defaults
        // to NOORDER, which on RAC lets each instance issue TXN_IDs from its own cached block.
        // The metastore reads an id above a reader's high water mark as not yet committed, so
        // losing the ordering here makes committed writers read as empty tables. OCR-2541.
        put(DbType.ORACLE, "ALTER TABLE \"TXNS\" MODIFY \"TXN_ID\" GENERATED BY DEFAULT AS IDENTITY (START WITH %s ORDER)");
  ...
  public List<String> getResetTxnSequenceStmts() {
    ...
    case POSTGRES:
      ...
      break;
    case ORACLE:
      // ORDER as above: restating the identity clause without it drops the ordering. OCR-2541.
      stmts.add("ALTER TABLE \"TXNS\" MODIFY \"TXN_ID\" GENERATED BY DEFAULT AS IDENTITY (START WITH 1 ORDER)");
      stmts.add("INSERT INTO \"TXNS\" (\"TXN_ID\", \"TXN_STATE\", \"TXN_STARTED\","
          + " \"TXN_LAST_HEARTBEAT\", \"TXN_USER\", \"TXN_HOST\")"
          + "  VALUES(0, 'c', 0, 0, '_', '_')");
      break;
