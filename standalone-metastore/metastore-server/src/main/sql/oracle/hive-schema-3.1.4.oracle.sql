...
------------------------------
-- TXN_ID is declared ORDER because Oracle defaults to NOORDER, and on RAC each instance then
-- caches its own block so TXN_IDs stop increasing with time. The metastore relies on that
-- ordering: a reader excludes every id above its own, so an out-of-order id leaves an already
-- committed writer above a later reader's high water mark and its table reads empty. OCR-2541.
-- Keep this comment outside the statement below: schematool joins the lines of one statement
-- with spaces, so a -- comment inside it would comment out the rest of the CREATE TABLE.
CREATE TABLE TXNS (
  TXN_ID NUMBER(19) GENERATED BY DEFAULT AS IDENTITY (ORDER) PRIMARY KEY,
